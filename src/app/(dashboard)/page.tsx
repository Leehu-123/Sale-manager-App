...
export default function DashboardPage() {
  ...
  const stats = [
    { label: 'Doanh thu tháng này', value: formatCurrency(data?.totalRevenue || 0), icon: TrendingUp, gradient: 'gradient-blue' },
    { label: 'Đã thanh toán', value: formatCurrency(data?.paidRevenue || 0), icon: DollarSign, gradient: 'gradient-green' },
    { label: 'Công nợ còn lại', value: formatCurrency(data?.unpaidRevenue || 0), icon: AlertCircle, gradient: 'gradient-red' },
    { label: 'Khách hàng mới', value: formatNumber(data?.newCustomers || 0), icon: UserPlus, gradient: 'gradient-purple' },
    ...
    { label: 'Giá trị Pipeline', value: formatCurrency(data?.pipelineValue || 0), icon: Briefcase, gradient: 'bg-gradient-to-br from-amber-500 to-amber-700' },
    ...
    { label: 'Tỷ lệ chốt đơn', value: `${(data?.closeRate || 0).toFixed(1)}%`, icon: CheckCircle, gradient: 'bg-gradient-to-br from-emerald-500 to-emerald-700' },
  ]
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-[#1e293b]">Tổng quan</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white rounded-xl p-4 border border-orange-100 flex items-center gap-4 hover:shadow-md transition-shadow">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">Task cần làm hôm nay</p>
          </div>
        </div>
        <div className="bg-white rounded-xl p-4 border border-red-100 flex items-center gap-4 hover:shadow-md transition-shadow">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">Task quá hạn</p>
          </div>
        </div>
        <div className="bg-white rounded-xl p-4 border border-blue-100 flex items-center gap-4 hover:shadow-md transition-shadow">
          <div className="p-3 bg-blue-100 rounded-lg">
            <Phone size={20} className="text-blue-600" />
          </div>
          <div>
            <p className="text-2xl font-bold text-blue-600">{data?.customersNeedFollowUp || 0}</p>
            <p className="text-sm text-gray-500">Khách cần chăm sóc lại</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl p-6 shadow-sm">
          <h3 className="font-semibold text-[#1e293b] mb-4">Doanh thu theo tháng</h3>
        ...
        {/* Pipeline Chart */}
        <div className="bg-white rounded-xl p-6 shadow-sm">
          <h3 className="font-semibold text-[#1e293b] mb-4">Pipeline theo giai đoạn</h3>
          <ResponsiveContainer width="100%" height={300}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-xl p-6 shadow-sm">
          <h3 className="font-semibold text-[#1e293b] mb-4">Top sản phẩm bán chạy</h3>
        ...
        {session?.user?.role !== 'SALES' && (
          <div className="bg-white rounded-xl p-6 shadow-sm">
            <h3 className="font-semibold text-[#1e293b] mb-4">Doanh thu theo nhân viên</h3>
