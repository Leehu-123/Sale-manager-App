...
export default function CustomerDetailPage() {
  ...
  if (!customer) return <div className="text-center py-12 text-surface-500">Không tìm thấy khách hàng</div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        <button onClick={() => router.push('/customers')} className="p-2 hover:bg-surface-100 rounded-lg"><ArrowLeft size={20} /></button>
        <div className="flex-1">
          ...
          <p className="text-sm text-surface-500 mt-1">{customer.code} · {CUSTOMER_TYPE_LABELS[customer.type]} · {CUSTOMER_SOURCE_LABELS[customer.source]}</p>
        </div>
      ...
      <div className="bg-white rounded-xl p-6 shadow-sm">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {customer.phone && (
            <div className="flex items-center gap-2 text-sm"><Phone size={14} className="text-surface-400" /><span>{customer.phone}</span></div>
          )}
          {customer.email && (
            <div className="flex items-center gap-2 text-sm"><Mail size={14} className="text-surface-400" /><span>{customer.email}</span></div>
          )}
          {customer.address && (
          ...
          {customer.assignedTo && (
            <div className="flex items-center gap-2 text-sm"><span className="text-surface-400">NV:</span><span className="font-medium">{customer.assignedTo.name}</span></div>
          )}
          {customer.nextFollowUpDate && (
            <div className="flex items-center gap-2 text-sm"><Calendar size={14} className="text-surface-400" /><span>Chăm sóc: {formatDate(customer.nextFollowUpDate)}</span></div>
          )}
        </div>
        {customer.productNeeds?.length > 0 && (
          <div className="mt-4 flex flex-wrap gap-2">
            {customer.productNeeds.map(p => <span key={p} className="px-2 py-1 bg-brand-50 text-brand-700 rounded text-xs">{p}</span>)}
          </div>
        )}
      </div>
      ...
      <div className="border-b border-surface-200">
        <div className="flex gap-0 overflow-x-auto">
          {tabs.map(tab => (
            <button key={tab.key} onClick={() => setActiveTab(tab.key)} className={`px-4 py-3 text-sm font-medium whitespace-nowrap border-b-2 transition-colors ${activeTab === tab.key ? 'border-[#1e3a5f] text-surface-900' : 'border-transparent text-surface-500 hover:text-surface-700'}`}>
              {tab.label}
            </button>
          ))}
        </div>
        ...
        {activeTab === 'interactions' && (
          <div className="space-y-4">
            {customer.interactions?.length > 0 ? customer.interactions.map(interaction => (
              <div key={interaction.id} className="bg-white rounded-xl p-4 shadow-sm border-l-4 border-brand-400 hover:shadow-md transition-shadow">
                <div className="flex items-start justify-between">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <span className="badge bg-brand-100 text-blue-800">{INTERACTION_TYPE_LABELS[interaction.type]}</span>
                      <span className="text-xs text-surface-400">{interaction.user.name}</span>
                    </div>
                    <p className="text-sm text-surface-900">{interaction.content}</p>
                    {interaction.result && <p className="text-sm text-surface-500 mt-1">Kết quả: {interaction.result}</p>}
                  </div>
                  <span className="text-xs text-surface-400 whitespace-nowrap">{formatDateTime(interaction.createdAt)}</span>
                </div>
              </div>
            )) : <div className="bg-white rounded-xl p-8 text-center text-surface-400"><Send size={40} className="mx-auto mb-2" /><p>Chưa có lịch sử chăm sóc</p></div>}
          </div>
        )}
          ...
          <div className="bg-white rounded-xl shadow-sm overflow-hidden">
            <table className="w-full data-table">
              ...
              <tbody>
                ...
                )) : <tr><td colSpan={4} className="text-center py-8 text-surface-400">Chưa có cơ hội</td></tr>}
              </tbody>
            </table>
          </div>
        )}
          ...
          <div className="bg-white rounded-xl shadow-sm overflow-hidden">
            <table className="w-full data-table">
              ...
              <tbody>
                ...
                )) : <tr><td colSpan={4} className="text-center py-8 text-surface-400">Chưa có báo giá</td></tr>}
              </tbody>
            </table>
          </div>
        )}
              ...
              <tbody>
                {customer.orders?.length > 0 ? customer.orders.map(o => (
                  <tr key={o.id} onClick={() => router.push(`/orders/${o.id}`)} className="cursor-pointer">
                    ...
                    <td><span className="badge bg-brand-100 text-blue-800">{ORDER_STATUS_LABELS[o.status]}</span></td>
                ...
                )) : <tr><td colSpan={5} className="text-center py-8 text-surface-400">Chưa có đơn hàng</td></tr>}
              </tbody>
            </table>
          </div>
        )}
            ...
            {customer.tasks?.length > 0 ? customer.tasks.map(t => (
              <div key={t.id} className="bg-white rounded-xl p-4 shadow-sm flex items-center justify-between hover:shadow-md transition-shadow">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-xs text-surface-400">{formatDate(t.dueDate)}</span>
                  <span className={`badge ${t.status === 'DONE' ? 'bg-green-100 text-green-800' : t.status === 'OVERDUE' ? 'bg-red-100 text-red-800' : 'bg-surface-100 text-surface-800'}`}>
                    {TASK_STATUS_LABELS[t.status]}
                  </span>
                </div>
              </div>
            )) : <div className="bg-white rounded-xl p-8 text-center text-surface-400">Chưa có công việc</div>}
          </div>
        )}
      </div>
      ...
      <Modal isOpen={showInteractionModal} onClose={() => setShowInteractionModal(false)} title="Ghi lịch sử chăm sóc">
        <form onSubmit={handleAddInteraction} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-surface-700 mb-1">Hình thức *</label>
            <select value={interactionForm.type} onChange={e => setInteractionForm({...interactionForm, type: e.target.value})} className="w-full border border-surface-300 rounded-lg px-3 py-2 text-sm">
              {Object.entries(INTERACTION_TYPE_LABELS).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
            </select>
          </div>
          <div>
            <label className="block text-sm font-medium text-surface-700 mb-1">Nội dung trao đổi *</label>
            <textarea value={interactionForm.content} onChange={e => setInteractionForm({...interactionForm, content: e.target.value})} required className="w-full border border-surface-300 rounded-lg px-3 py-2 text-sm" rows={4} placeholder="Mô tả nội dung cuộc trao đổi..." />
          </div>
          <div>
            <label className="block text-sm font-medium text-surface-700 mb-1">Kết quả</label>
            <input value={interactionForm.result} onChange={e => setInteractionForm({...interactionForm, result: e.target.value})} className="w-full border border-surface-300 rounded-lg px-3 py-2 text-sm" placeholder="Kết quả cuộc trao đổi" />
          </div>
          <div>
            <label className="block text-sm font-medium text-surface-700 mb-1">Ngày hẹn chăm sóc tiếp</label>
            <input type="date" value={interactionForm.nextFollowUpDate} onChange={e => setInteractionForm({...interactionForm, nextFollowUpDate: e.target.value})} className="w-full border border-surface-300 rounded-lg px-3 py-2 text-sm" />
          </div>
          <div className="flex gap-3 pt-2">
            <button type="button" onClick={() => setShowInteractionModal(false)} className="flex-1 py-2 border border-surface-300 rounded-lg text-sm">Hủy</button>
